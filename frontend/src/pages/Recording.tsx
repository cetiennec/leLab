...
type Phase = "preparing" | "settling" | "recording" | "resetting" | "completed";
...
const Recording = () => {
      ...
      try {
        ...
        if (prev !== real) {
          if (real === "recording" && prev !== null) {
            playRecordingStartCue();
          } else if (real === "resetting" || real === "settling") {
            playResetStartCue();
          }
  ...
  const handleExitEarly = useCallback(async () => {
    ...
    const next: Phase | null =
      ...
      realPhase === "resetting"
        ? (recordingConfig.settling_time_s ?? 0) > 0
          ? "settling"
          : "recording"
        :
      realPhase === "settling" ? "recording" : null;
  ...
  const phaseTimeLimit =
    currentPhase === "recording"
      ...
      ? recordingConfig.settling_time_s ?? backendStatus.phase_time_limit_s ?? 0
  ...
  const getStatusText = () => {
    ...
    if (currentPhase === "settling") return `GET IN POSITION — EPISODE ${currentEpisode}`;
  ...
  const phaseColor =
    currentPhase === "recording"
      ...
      : currentPhase === "resetting" || currentPhase === "settling"
